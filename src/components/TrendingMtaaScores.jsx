...
const MtaaScoreGrid = ({ score }) => {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 mt-2">
      ...
      {/* Security - ✅ NOW USES CONSISTENT COLOR LOGIC */}
...
// ... (Rest of MtaaFlipCard and TrendingMtaaScores stays the same as previous step)
// Re-paste logic if needed, but the key fix was MtaaScoreGrid above.
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setItems(aggregated.sort((a, b) => b.rating - a.rating).slice(0, 3));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
