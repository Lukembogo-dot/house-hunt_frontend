    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60]">
      <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[95vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {isAdmin && step === 1 && (
             <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-800 mb-4 animate-pulse">
                <h4 className="text-xs font-bold text-red-700 dark:text-red-300 uppercase mb-2 flex items-center gap-1">
                   <FaUserSecret /> Admin Override (Shadow Profile)
                </h4>
                <div className="grid grid-cols-2 gap-3">
                   <div>
                      <label className="text-[10px] uppercase font-bold text-gray-500 block">Custom Alias</label>
                      <input 
                        type="text" 
                        placeholder="e.g. Resident #42" 
                   ...
                   <div>
                      <label className="text-[10px] uppercase font-bold text-gray-500 block">Backdate Review</label>
                      <input 
                        type="date" 
                        className="w-full p-2 text-sm border rounded bg-white dark:bg-gray-800 dark:text-white"
            ...
            <div className="space-y-6 animate-in slide-in-from-right duration-300">
                        ...
                        <div className="mt-3 bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg border border-blue-100 dark:border-blue-800">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {['1-2 Days/Week', '3-4 Days/Week', 'Weekends Only', 'Weekdays Only'].map(opt => (
                                    <button key={opt} onClick={() => setMtaaData({...mtaaData, waterRationingSchedule: opt})} className={`text-[10px] px-3 py-1.5 rounded-full border transition ${mtaaData.waterRationingSchedule === opt ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-100'}`}>{opt}</button>
                                ))}
                            </div>
                        </div>
                   )}
               ...
               <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SECURITY_OPTIONS.map(feat => (
                      <span key={feat} onClick={() => toggleArrayItem('securityFeatures', feat)} className={`text-xs px-2 py-1 rounded-full border cursor-pointer select-none ${mtaaData.securityFeatures.includes(feat) ? 'bg-green-100 text-green-700 border-green-300 font-bold' : 'bg-gray-50 text-gray-500 dark:bg-gray-700'}`}>{feat}</span>
                    ))}
                  </div>
               </section>
               ...
               <section>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                     ...
                     <div>
                        <label className="text-xs mb-1 block dark:text-gray-300">Off-Peak Fare (KES)</label>
                        <input type="number" className="w-full p-2 text-sm border rounded-lg dark:bg-gray-700 dark:text-white" value={mtaaData.matatuFareOffPeak} onChange={e => setMtaaData({...mtaaData, matatuFareOffPeak: e.target.value})} />
                     </div>
                  </div>
               </section>
