...
const AddResidencyModal = ({ isOpen, onClose, refresh, initialData = null }) => {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      // ✅ MAP VALUES TO MATCH BACKEND SCHEMA EXACTLY
      let mappedNoise = mtaaData.noiseLevel;
      if (mappedNoise === 'Noisy') mappedNoise = 'Noisy (Club/Road)'; // Schema compliance
      ...
      const payload = {
        ...
        utilities: {
          ...
          // Combine consistency for backend backward compatibility or send separate
          waterConsistency: mtaaData.waterConsistency === '24/7' ? '24/7 Council Water' : `${mtaaData.waterConsistency} (${mtaaData.waterSource})`, // Sent purely to avoid enum errors
        ...
        accessibility: {
          ...
          matatuAvailability: mtaaData.matatuAvailability, 
        ...
        amenities: { 
          ...
          noiseLevel: mappedNoise, // ✅ Using Mapped Noise Value
          noiseSources: mtaaData.noiseSources
        },
        // We merge extra tags into lifestyleTags for searching
  ...
  // ✅ Updated Enum Options to Match Backend Schema Exactly
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60]">
      <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[95vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {/* STEP 2: The Detailed Experience */}
          {step === 2 && (
            <div className="space-y-6 animate-in slide-in-from-right duration-300">
               
               {/* 1. UTILITIES (Updated: Separated Sections) */}
               <section>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                     ...
                     <div>
                        ...
                        <select className="w-full p-2 text-sm border rounded-lg dark:bg-gray-700 dark:text-white" value={mtaaData.waterSource} onChange={e => setMtaaData({...mtaaData, waterSource: e.target.value})}>
                          <option>Council Water</option>
                          <option>Borehole (Treated)</option>
                          <option>Borehole (Untreated)</option>
                          <option>Mixed (Council + Borehole)</option>
                        </select>
                     </div>
                     ...
                     <div className="col-span-1 md:col-span-2">
                        <label className="text-xs mb-1 block dark:text-gray-300">Reliability</label>
                        <div className="grid grid-cols-3 gap-2">
                            {['24/7', 'Rationed (Weekly)', 'Frequent Shortages'].map(opt => (
                                <button
                                    key={opt}
                                    onClick={() => setMtaaData({...mtaaData, waterConsistency: opt})}
                                    className={`text-xs p-2 rounded-lg border text-center ${mtaaData.waterConsistency === opt ? 'bg-blue-600 text-white' : 'bg-gray-50 dark:bg-gray-700 dark:text-gray-300'}`}
                                >
                                    {opt}
                                </button>
                            ))}
                   ...
                   {/* Rationing Schedule (Conditional) */}
                   {mtaaData.waterConsistency.includes('Rationed') && (
                        <div className="mt-3 bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg border border-blue-100 dark:border-blue-800 animate-in slide-in-from-top-1">
                            <label className="text-xs font-bold text-blue-700 dark:text-blue-300 mb-1 block">Rationing Schedule (How many days?):</label>
                            <div className="flex gap-2 flex-wrap">
                                {['1-2 Days/Week', '3-4 Days/Week', 'Weekends Only', 'Weekdays Only'].map(opt => (
                                    <button
                                    key={opt}
                                    onClick={() => setMtaaData({...mtaaData, waterRationingSchedule: opt})}
                                    className={`text-[10px] px-3 py-1.5 rounded-full border transition ${
                                        mtaaData.waterRationingSchedule === opt
               ...
               <section>
                  ...
                  <div className="mb-3">
                      ...
                      <select className="w-full p-2 text-sm border rounded-lg dark:bg-gray-700 dark:text-white" value={mtaaData.safeAtNight} onChange={e => setMtaaData({...mtaaData, safeAtNight: e.target.value})}>
                        ...
                        <option>Unsafe (Avoid night walking)</option>
                      </select>
                  </div>
                  
                  <label className="text-xs text-gray-500 mb-1 block">Building Features (Select all):</label>
                  <div className="flex flex-wrap gap-2">
                    {['CCTV', '24h Guard', 'Night Guard', 'Biometric', 'Electric Fence', 'Perimeter Wall', 'Locked Gate'].map(feat => (
                      <span key={feat} onClick={() => toggleArrayItem('securityFeatures', feat)} className={`text-xs px-2 py-1 rounded-full border cursor-pointer select-none ${mtaaData.securityFeatures.includes(feat) ? 'bg-green-100 text-green-700 border-green-300 font-bold' : 'bg-gray-50 text-gray-500 dark:bg-gray-700'}`}>
                        {feat}
                      </span>
                    ))}
                  </div>
               ...
               <section>
                  ...
                  <div className="flex gap-2 mb-3">
                      {['Silent', 'Moderate', 'Noisy'].map(lvl => (
                          <button
                            key={lvl}
                            onClick={() => setMtaaData({...mtaaData, noiseLevel: lvl})}
                            className={`flex-1 py-2 text-xs rounded-lg border ${mtaaData.noiseLevel === lvl ? 'bg-orange-500 text-white border-orange-600' : 'bg-gray-50 dark:bg-gray-700 dark:text-white'}`}
                          >
                              {lvl}
                          </button>
                      ))}
                  ...
                  {mtaaData.noiseLevel !== 'Silent' && (
                      <div className="bg-orange-50 dark:bg-orange-900/10 p-3 rounded-lg border border-orange-100 dark:border-orange-800">
                          <label className="text-xs font-bold text-orange-800 dark:text-orange-200 mb-2 flex items-center gap-1">
                              <FaVolumeUp /> Primary Noise Sources:
                          </label>
                          <div className="flex flex-wrap gap-2">
                              {['Road/Traffic', 'Club/Bar', 'Church/Mosque', 'Construction', 'Children Playing', 'Neighbors'].map(src => (
                                  <span key={src} onClick={() => toggleArrayItem('noiseSources', src)} className={`text-[10px] px-2 py-1 rounded border cursor-pointer ${mtaaData.noiseSources.includes(src) ? 'bg-orange-200 text-orange-900 border-orange-300' : 'bg-white text-gray-600 dark:bg-gray-800 dark:text-gray-300'}`}>
                                      {src}
                                  </span>
                              ))}
                          </div>
                    ...
                    <div>
                      {!isCustomInternet ? (
                        ...
                        <input 
                          ...
                          className="w-full p-2 text-sm border rounded-lg border-blue-500 dark:bg-gray-700 dark:text-white"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                     <div>
                        ...
                        <select className="w-full p-2 text-sm border rounded-lg dark:bg-gray-700 dark:text-white" value={mtaaData.matatuAvailability} onChange={e => setMtaaData({...mtaaData, matatuAvailability: e.target.value})}>
                            <option>24/7</option>
                            <option>Until Midnight</option>
                            <option>Until 10pm</option>
                            <option>Irregular</option>
                        </select>
                     </div>
                     <div>
                        <label className="text-xs mb-1 block dark:text-gray-300">Peak Fare (KES)</label>
                        <input type="number" className="w-full p-2 text-sm border rounded-lg dark:bg-gray-700 dark:text-white" value={mtaaData.matatuFarePeak} onChange={e => setMtaaData({...mtaaData, matatuFarePeak: e.target.value})} />
                     </div>
                  </div>
               </section>
               ...
               <section>
                  ...
                  <div className="flex items-center justify-between mb-3 bg-gray-50 dark:bg-gray-700 p-2 rounded-lg">
                     ...
                     <button 
                       onClick={() => setMtaaData({...mtaaData, supermarketNearby: !mtaaData.supermarketNearby})}
                       className={`px-3 py-1 rounded text-xs font-bold ${mtaaData.supermarketNearby ? 'bg-green-600 text-white' : 'bg-gray-300 text-gray-600'}`}
                     >
                       {mtaaData.supermarketNearby ? 'YES' : 'NO'}
                     </button>
                  </div>
