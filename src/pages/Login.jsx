...
const Login = () => {
      ...
      if (data.found) {
        ...
        if (data.isShadow) {
          ...
          setClaimDetails({
             ...
             name: data.name,
             whatsappNumber: identifier.includes('@') ? '' : identifier, // Best guess
  ...
  const handlePasswordLogin = async (e) => {
    ...
    try {
      // Use the verified email from Step 1 for actual login
      const response = await apiClient.post(
        "/auth/login",
        { email: userData.email, password }, // userData.email comes from check-user response
  ...
  const handleClaimSubmit = async (e) => {
    ...
    try {
      ...
      // Disable form interactions after success
    } catch (err) {
